...
for (const [name, query] of Object.entries(ADMIN_QUERIES) as Array<[string, AdminQuery]>) {
  const fail = (reason: string) => {
    throw new Error(`.bolt/admin.json query "${name}": ${reason}`);
  };

  if (!/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(name)) fail('name must be letters, digits and underscores');
  ...
  for (let n = 1; n <= count; n++) {
    ...
    if (query.preview !== undefined && !pattern.test(query.preview)) fail(`preview must reference $${n}`);
  }
  ...
  if (query.description && query.description.length > 200) fail('description is longer than 200 characters');
  if (query.readOnly === false && /^select\b/i.test(query.sql)) fail('readOnly: false on a select');
}
...
if (process.argv.includes('--check')) {
  ...
  console.log(`wrote .bolt/admin.json (${Object.keys(ADMIN_QUERIES).length} queries)`);
}
