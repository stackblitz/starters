...
description: >-
  ...
  the public site (not the /admin UI).
---
...
themes: `classic`, `editorial`, `minimal`. The active theme is
`cms_settings.theme`, chosen in **Admin → Appearance**, and applied by
`app/routes/site/layout.tsx` on the `<html>` element.
...
4. Optionally set it as default: `update cms_settings set value =
   '"<name>"' where key = 'theme';` in a new migration.
...
- Don't use `--color-bolt-ds-*` tokens on the public site; those are
  the Bolt design-system tokens used only by `/admin`.
