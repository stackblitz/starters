...
async function renderSlidePng(slide: SlideData, w: number, h: number, pixelRatio: number): Promise<string> {
  ...
  try {
    ...
    await sleep(60)
    try { await (doc as Document & { fonts?: { ready: Promise<unknown> } }).fonts?.ready } catch { /* offline fonts are fine */ }
    await Promise.all(
    ...
    const png = await toPng(mount, { width: w, height: h, pixelRatio, backgroundColor: bg })
    root.unmount()
    return png
  } finally {
    iframe.remove()
...
export async function exportPdf(
  slides: SlideData[], title: string, onProgress: (msg: string) => void,
): Promise<void> {
  const W = 1280, H = 720
  ...
  pdf.save(`${(title || 'deck').replace(/[^\w\- ]+/g, '').trim() || 'deck'}.pdf`)
}
