...
export default function PresentApp() {
  ...
  useEffect(() => { load().catch(() => { /* the gate explains why */ }) }, [load])
  useEffect(() => { document.title = deck.title || 'Slides' }, [deck.title])
  useEffect(() => { applyFont(deck.font) }, [deck.font])
  useEffect(() => { applyAccent(deck.accent) }, [deck.accent])

  if (denied) return <Gate />
  if (!loaded) return <div className="boot-screen">Loading deck…</div>
  if (!slides.length) return <div className="boot-screen">This deck has no slides yet — add some in the editor.</div>
  ...
  return (
    <Deck
      ...
      onNotes={(i, text) => { const s = slides[i]; if (s) patchSlide(s.id, { notes: text }) }}
      navLabel={(i) => {
        const s = slides[i]
        if (!s) return undefined
        const raw = s.props?.title ?? s.props?.text ?? s.props?.value ?? s.props?.name
        const plain = typeof raw === 'string' ? stripRich(raw).replace(/\s+/g, ' ').trim() : ''
        return plain ? (plain.length > 52 ? plain.slice(0, 52) + '…' : plain) : LAYOUTS[s.layout]?.label
      }}
    >
      {slides.map((s) => (
        <SlideView
