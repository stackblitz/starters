...
const PALETTE = ['#ffffff', '#9aa3b2', '#1688FC', '#4fe5b0', '#eab308', '#fb923c', '#f87171', '#a78bfa', '#f472b6', '#34d399']

/* the five formats: how to create each wrapper element, how to recognize it */
const WRAPPERS: Record<string, { make: () => HTMLElement; match: (el: HTMLElement) => boolean }> = {
  '**': {
  ...
  '==': {
    make: () => { const s = document.createElement('span'); s.className = 'accent-text'; return s },
    match: (el) => el.classList.contains('accent-text'),
  },
  '++': {
    make: () => { const s = document.createElement('span'); s.dataset.size = 'up'; s.style.fontSize = '1.3em'; return s },
    match: (el) => el.dataset.size === 'up',
  },
  '~~': {
    make: () => { const s = document.createElement('span'); s.dataset.size = 'down'; s.style.fontSize = '0.75em'; return s },
...
function serialize(node: Node): string {
  ...
  if (node.dataset.color && COLOR_RE.test(node.dataset.color)) return `{c:${node.dataset.color}}${inner}{/c}`
...
const IconBold = (
  <svg viewBox="0 0 20 20" width="15" height="15" aria-hidden>
    <path d="M6.2 3.6h4.6a3.1 3.1 0 0 1 0 6.2H6.2zM6.2 9.8h5.4a3.3 3.3 0 0 1 0 6.6H6.2z" fill="none" stroke="currentColor" strokeWidth="1.9" strokeLinejoin="round" />
  </svg>
)
const IconItalic = (
  <svg viewBox="0 0 20 20" width="15" height="15" aria-hidden>
    <path d="M8.6 3.6h7M4.4 16.4h7M12.4 3.6l-4.4 12.8" fill="none" stroke="currentColor" strokeWidth="1.9" strokeLinecap="round" />
  </svg>
)
const IconClear = (
  <svg viewBox="0 0 20 20" width="15" height="15" aria-hidden>
    <path d="M5 4.6h10M10 4.6l-2.2 10.8" fill="none" stroke="currentColor" strokeWidth="1.9" strokeLinecap="round" />
    <path d="M12.4 12.4l4 4M16.4 12.4l-4 4" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" />
  </svg>
)
const alignIcon = (kind: 'l' | 'c' | 'r') => {
  const x2 = (w: number) => (kind === 'l' ? [3, 3 + w] : kind === 'r' ? [17 - w, 17] : [10 - w / 2, 10 + w / 2])
  return (
    <svg viewBox="0 0 20 20" width="15" height="15" aria-hidden>
      {[[14, 4.5], [9, 9.5], [12, 14.5]].map(([w, y], i) => {
        const [a, b] = x2(w)
        return <path key={i} d={`M${a} ${y}h${b - a}`} stroke="currentColor" strokeWidth="1.9" strokeLinecap="round" />
      })}
    </svg>
  )
}
...
function FormatMenu({ bar, inline, onBeforeAction, onApply, onClear, onSize, onColor, onAlign }: {
  bar: Bar
  ...
  const step = (dir: 1 | -1) => onSize(clampEm(Math.round((bar.sizeEm + dir * 0.1) * 10) / 10))
  const btn = (props: { title: string; cls?: string; on?: boolean; act: () => void; children: ReactNode; disabled?: boolean }) => (
    <button
      className={'fmt-btn ' + (props.cls ?? '') + (props.on ? ' on' : '')}
      title={props.title}
      disabled={props.disabled}
      onMouseDown={(e) => { e.preventDefault(); if (!props.disabled) props.act() }}
    >
      {props.children}
    </button>
  )
  const body = (
    <div
      className={inline ? 'fmt-menu fmt-inline' : 'fmt-menu fmt-bar' + (bar.below ? ' below' : '')}
    ...
    >
      {btn({ title: 'Smaller (−10%)', act: () => step(-1), disabled: bar.sizeEm <= 0.4, children: '−' })}
      <span className="fmt-size">{pct}%</span>
      {btn({ title: 'Larger (+10%)', act: () => step(1), disabled: bar.sizeEm >= 4, children: '+' })}
      ...
      <span className="fmt-sep" />
      {btn({ title: 'Bold (⌘B)', on: bar.marks.includes('**'), act: () => onApply('**'), children: IconBold })}
      {btn({ title: 'Italic (⌘I)', on: bar.marks.includes('_'), act: () => onApply('_'), children: IconItalic })}
      <span className="fmt-sep" />
      <span className="fmt-drop">
        {btn({
          title: 'Alignment',
          on: bar.align !== null,
          act: () => { setAlignOpen((v) => !v); setColorOpen(false) },
          children: alignIcon(bar.align ?? 'l'),
        })}
        {alignOpen && (
          <span className="fmt-pop" onMouseDown={(e) => e.preventDefault()}>
            {(['l', 'c', 'r'] as const).map((a) => btn({
              title: { l: 'Align left', c: 'Align center', r: 'Align right' }[a],
              on: bar.align === a,
              act: () => { onAlign(bar.align === a ? null : a); setAlignOpen(false) }, // active again → layout default
      ...
      <span className="fmt-sep" />
      {btn({ title: 'Clear formatting', act: () => { setColorOpen(false); onClear() }, children: IconClear })}
        ...
        <div className="color-pop" onMouseDown={(e) => e.preventDefault()}>
          ...
          <div className="color-row">
            <button
              ...
              onMouseDown={(e) => { e.preventDefault(); onColor(null); setColorOpen(false) }}
            />
            <button
              className={'color-swatch' + (bar.color === 'accent' ? ' on' : '')}
              title="Accent"
              style={{ background: 'var(--primary)' }}
              onMouseDown={(e) => { e.preventDefault(); onColor('accent'); setColorOpen(false) }}
            />
          </div>
          ...
          <div className="color-row">
            {PALETTE.map((c) => (
              <button
                ...
                title={c}
                style={{ background: c }}
                onMouseDown={(e) => { e.preventDefault(); onColor(c); setColorOpen(false) }}
              />
            ))}
            <label className="color-swatch color-custom" title="Custom color">
              ...
              <input
                type="color"
                onChange={(e) => { onColor(e.target.value); setColorOpen(false) }}
              />
            </label>
          </div>
        </div>
...
export default function T({
  path, placeholder = 'Edit…', block, inlineBar,
}: {
  ...
  const value: string = storeValue ?? String(getPath(ctxSlide?.props ?? {}, path) ?? '')
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      if (s && s.rangeCount > 0 && el.contains(s.anchorNode)) {
        ...
        while (n && n !== el) {
          if (n instanceof HTMLElement) {
            for (const [marker, w] of Object.entries(WRAPPERS)) if (w.match(n)) marks.push(marker)
            if (n.dataset.fs && sizeEm === 1) sizeEm = clampEm(parseFloat(n.dataset.fs))
      ...
      setBar({
        ...
        sizeEm,
        color,
        align: (el.querySelector('[data-align]') as HTMLElement | null)?.dataset.align as 'l' | 'c' | 'r' | undefined ?? null,
      })
    }
  ...
  if (!editable || !slideId) return <span style={blockStyle}>{renderRich(value)}</span>
  ...
  const commit = () => {
    ...
    let raw = balanceLines(
      Array.from(el.childNodes).map(serialize).join('').replace(/^\n/, '').replace(/\n$/, ''),
    )
  ...
  const ensureRange = () => {
    ...
    if (saved && el.contains(saved.commonAncestorContainer) && !saved.collapsed) {
      s.removeAllRanges()
      s.addRange(saved)
      return
    }
  ...
  const findAncestor = (test: (el: HTMLElement) => boolean): HTMLElement | null => {
    const el = ref.current
    const s = document.getSelection()
  ...
  const setSize = (em: number) => {
    ...
    if (cur) {
      if (Math.abs(em - 1) < 0.05) { unwrapEl(cur); return }
    ...
    const legacy = findAncestor((n) => n.dataset.size === 'up' || n.dataset.size === 'down')
  ...
  const setColor = (color: string | null) => {
    ensureRange()
    const cur = findAncestor((n) => !!n.dataset.color || n.classList.contains('accent-text'))
    if (cur) {
      if (!color) { unwrapEl(cur); return }
  ...
  const clearFormat = () => {
    ...
    while (n && n !== el) {
      ...
      if (n instanceof HTMLElement && (n.dataset.color || n.dataset.fs || Object.values(WRAPPERS).some((w) => w.match(n as HTMLElement)))) {
  ...
  const onKeyDown = (e: KeyboardEvent) => {
    ...
    if (e.key === 'Enter' && !block) { e.preventDefault(); ref.current?.blur() }
    if ((e.metaKey || e.ctrlKey) && e.key.toLowerCase() === 'b') { e.preventDefault(); applyMarker('**') }
    if ((e.metaKey || e.ctrlKey) && e.key.toLowerCase() === 'i') { e.preventDefault(); applyMarker('_') }
  ...
  return (
    <>
      ...
      <span
        ...
        style={blockStyle}
        className={'t-edit' + (focused ? ' focused' : '') + (empty ? ' empty' : '')}
        contentEditable
        suppressContentEditableWarning
        spellCheck={false}
        data-placeholder={placeholder}
        onFocus={() => { setFocused(true); setDomEmpty(!ref.current?.textContent) }}
        onBlur={commit}
