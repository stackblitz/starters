...
export default function LiCtl({ path, index, blank, children }: {
  ...
  const append = () => {
    const a = arr()
    const tpl = blank !== undefined ? structuredClone(blank) : (typeof a[index] === 'string' ? '' : {})
    a.push(tpl)
    commit(a)
  ...
  const startDrag = (e: React.MouseEvent) => {
    ...
    let target: { el: HTMLElement; index: number; side: 'before' | 'after' } | null = null
    const clearTarget = () => {
      target?.el.classList.remove('li-over-before', 'li-over-after')
      target = null
    }
    const onMove = (ev: MouseEvent) => {
      const hit = document.elementsFromPoint(ev.clientX, ev.clientY).find(
        (el): el is HTMLElement =>
          el instanceof HTMLElement && el.dataset.liKey === listKey && Number(el.dataset.liIndex) !== index,
      )
  ...
  return (
    <span
      ref={(el: HTMLElement | null) => { wrapRef.current = el }}
      ...
      onBlur={() => {
        setTimeout(() => {
          ...
          if (useStore.getState().activeList === listKey && !document.activeElement?.closest?.('.li')) {
            setActiveList(null)
          }
        }, 250)
      }}
    ...
    >
      ...
      {isLast && (
        <span
          ...
          title="Add item"
          onClick={(e) => { e.stopPropagation(); append() }}
          onKeyDown={(e) => { if (e.key === 'Enter') { e.stopPropagation(); append() } }}
        >+</span>
      )}
      {menu && createPortal(
        <ContextMenu
