...
export default function CodeEditor({ slide }: { slide: SlideData }) {
  ...
  const [geom, setGeom] = useState<{ left: number; top: number; width: number; height: number; font: string; lineHeight: string } | null>(null)
  ...
  const commit = () => {
    if (slideId && draft !== null && draft !== slide.props.code) setProp(slideId, 'code', draft)
    setDraft(null)
  }

  const highlight = String(slide.props.highlight ?? '').split(',').map((n) => parseInt(n.trim(), 10)).filter(Number.isFinite)

  return (
    <div className="code-edit" ref={wrapRef}>
      {/* the visual — re-renders live from the draft */}
      <CodeWindow title={(<T path="filename" placeholder="file.ts" />) as unknown as string} code={code} highlight={highlight} />

      {geom && (
        <textarea
          ref={taRef}
          className="code-ta"
          style={{ left: geom.left, top: geom.top, width: geom.width, height: geom.height, font: geom.font, lineHeight: geom.lineHeight }}
          value={code}
          wrap="off"
          spellCheck={false}
          ...
          onKeyDown={(e) => {
            ...
            if (e.key === 'Escape') { e.preventDefault(); taRef.current?.blur() }
            if (e.key === 'Tab') {
              e.preventDefault()
