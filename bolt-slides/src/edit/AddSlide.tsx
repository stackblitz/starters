...
export default function AddSlide({ at, onClose }: { at?: number; onClose: () => void }) {
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal add-slide" onClick={(e) => e.stopPropagation()}>
        <div className="modal-head">
          <h3>New slide</h3>
          <button className="ghost-btn" onClick={onClose}>Esc</button>
        </div>
        <div className="add-pills">
          <button className={'add-pill' + (filter === null ? ' on' : '')} onClick={() => setFilter(null)}>All</button>
          {LAYOUT_GROUPS.map((g) => (
            <button key={g.title} className={'add-pill' + (filter === g.title ? ' on' : '')} onClick={() => setFilter(g.title)}>{g.title}</button>
          ))}
        </div>
        {groups.map((g) => (
          <div key={g.title} className="add-group">
            ...
            <div className="add-grid">
              {g.types.map((t) => LAYOUTS[t]).filter(Boolean).map((l) => {
            ...
            <div
              ...
              role="button"
              tabIndex={0}
              onClick={() => { addSlide(l.type, structuredClone(l.defaults), at, structuredClone(PREVIEW_BGS[i % PREVIEW_BGS.length])); onClose() }}
              onKeyDown={(e) => { if (e.key === 'Enter') { addSlide(l.type, structuredClone(l.defaults), at, structuredClone(PREVIEW_BGS[i % PREVIEW_BGS.length])); onClose() } }}
            >
              <span className="add-card-preview"><MiniSlide slide={previewSlide(l, i)} /></span>
