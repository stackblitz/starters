  ...
  return (
    <div className="cnvp-swatches">
      ...
      <input
        ...
        title="Custom color"
        value={
          /^#[0-9a-f]{6}$/i.test(value ?? '') ? (value as string) : '#1688fc'
        }
...
export default function LayoutDataSheet({
  ...
  const barsSheet = (barsPath: string) => {
    const bars: { label: string; value: number }[] = Array.isArray(p[barsPath])
      ? p[barsPath]
      : [];
    body = (
      <div className="cnv-data-grid bars">
        ...
        {bars.map((b, r) => (
          <div key={r} className="cnv-data-row">
            ...
            <input
              value={b.label}
              onChange={(e) => set(`${barsPath}.${r}.label`, e.target.value)}
            />
            <input
              inputMode="decimal"
              value={String(b.value)}
              onChange={(e) =>
                set(`${barsPath}.${r}.value`, Number(e.target.value) || 0)
              }
            />
            {x({
              title: 'Remove row',
    ...
    actions = (
      <button
        className="ghost-btn"
        onClick={() =>
          set(barsPath, [...bars, { label: `Q${bars.length + 1}`, value: 10 }])
        }
      >
        + Row
      </button>
  ...
  const pointsSheet = (pointsPath: string) => {
    ...
    actions = (
      <button className="ghost-btn" onClick={() => setPts([...pts, '10'])}>
        + Row
      </button>
    );
  };
    ...
    body = (
      <div className="cnv-data-grid donut">
        ...
        <div className="cnv-data-row">
          ...
          <input
            value={p.donutLabel ?? ''}
            onChange={(e) => set('donutLabel', e.target.value)}
          />
        </div>
      </div>
    );
  ...
  if (kind === 'bars' || kind === 'i-bars') barsSheet('bars');
    ...
    const donuts: { value: number; label: string }[] = Array.isArray(p.donuts)
      ? p.donuts
      : [];
    body = (
      <div className="cnv-data-grid bars">
        <span className="cnv-data-th" />
        <span className="cnv-data-th">Label</span>
        <span className="cnv-data-th">Percent</span>
        <span className="cnv-data-th" />
        {donuts.map((d, r) => (
          <div key={r} className="cnv-data-row">
            <span className="cnv-data-no">{r + 1}</span>
            <input
              value={d.label}
              onChange={(e) => set(`donuts.${r}.label`, e.target.value)}
            />
            <input
              inputMode="decimal"
            ...
            {x({
              title: 'Remove row',
              disabled: donuts.length <= 1,
              onClick: () =>
                set(
                  'donuts',
                  donuts.filter((_, j) => j !== r)
                ),
            })}
          </div>
        ))}
    ...
    actions = (
      <button
        className="ghost-btn"
        onClick={() => set('donuts', [...donuts, { label: 'New', value: 50 }])}
      >
        + Row
      </button>
    );
    ...
    const series: { label: string; values: string }[] = Array.isArray(p.series)
      ? p.series
      : [];
    body = (
      <div
        ...
        style={{
          gridTemplateColumns: `30px minmax(110px, 1fr) repeat(${cats.length}, minmax(80px, 1fr)) 30px`,
        }}
      >
        ...
        {series.map((sr, r) => {
          ...
          return (
            <div key={r} className="cnv-data-row">
              ...
              <input
                value={sr.label}
                onChange={(e) => set(`series.${r}.label`, e.target.value)}
              />
              {cats.map((_, ci) => (
                <input
                  key={ci}
                  ...
                  value={vals[ci] ?? ''}
                  onChange={(e) => {
                    const next = cats.map((_, j) =>
                      j === ci ? e.target.value : vals[j] ?? '0'
                    );
    ...
    const lines: { label: string; points: string }[] = Array.isArray(p.lines)
      ? p.lines
      : [];
    ...
    body = (
      <div
        className="cnv-data-grid table"
        style={{
          gridTemplateColumns: `30px minmax(110px, 1fr) repeat(${n}, minmax(64px, 1fr)) 30px`,
        }}
      >
        <span className="cnv-data-no" />
        <span className="cnv-data-th">Line</span>
        {Array.from({ length: n }, (_, ci) => (
          <span key={ci} className="cnv-data-th">
            {ci + 1}
          </span>
        ))}
        <span />
        {lines.map((l, r) => {
          ...
          return (
            <div key={r} className="cnv-data-row">
              ...
              <input
                value={l.label}
                onChange={(e) => set(`lines.${r}.label`, e.target.value)}
              />
              {Array.from({ length: n }, (_, ci) => (
                <input
                  key={ci}
    ...
    actions = (
      <button
        ...
        onClick={() =>
          set('lines', [
            ...lines,
            { label: `Line ${lines.length + 1}`, points: '10 | 20 | 15' },
          ])
        }
      >
        + Line
      ...
      >
        ...
        {cols.map((c, ci) => (
          <span key={ci} className="cnv-data-colhead">
            <input
              value={c}
              onChange={(e) => set(`columns.${ci}`, e.target.value)}
            />
            {x({
              title: 'Remove column',
          ...
          <div key={r} className="cnv-data-row">
            ...
            {cols.map((_, c) => (
              <input
                key={c}
                value={row[c] ?? ''}
                onChange={(e) => set(`rows.${r}.${c}`, e.target.value)}
              />
            ))}
            {x({
              title: 'Remove row',
      ...
      >
        ...
        {cols.map((c, ci) => (
          <span key={ci} className="cnv-data-colhead">
            <input
              ...
              onChange={(e) => set(`cols.${ci}`, e.target.value)}
            />
            {ci > 0 &&
              x({
                title: 'Remove column',
        ...
        {rows.map((row, r) => (
          <div key={r} className="cnv-data-row">
            ...
            <input
              value={row.label}
              onChange={(e) => set(`rows.${r}.label`, e.target.value)}
            />
            {cols.slice(1).map((_, c) => (
              <CmpCell
                key={c}
                value={row.values[c] ?? true}
                onChange={(nv) => set(`rows.${r}.values.${c}`, nv)}
              />
            ))}
            {x({
              title: 'Remove row',
      ...
      <>
        <button
          ...
          onClick={() =>
            set('rows', [
              ...rows,
              { label: 'New row', values: cols.slice(1).map(() => true) },
            ])
          }
        >
          + Row
        ...
        <button
          ...
          onClick={() => {
            ...
            set(
              'rows',
              rows.map((r) => ({ ...r, values: [...r.values, true] }))
            );
          }}
        >
          + Column
    ...
    head = (
      <span className="fld-hint">
        Click a cell to toggle it — abc switches the cell to text.
      </span>
    );
  }
