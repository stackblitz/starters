...
export const GRADIENTS: { from: string; to: string }[] = [
  ...
  { from: '#04121f', to: '#0e4f63' }, { from: '#101426', to: '#37175e' },
  { from: '#071a17', to: '#0e5246' }, { from: '#10131a', to: '#3a4356' },
  { from: '#1d1030', to: '#7b2c5e' }, { from: '#23100d', to: '#8f3a1e' },
  { from: '#1e1405', to: '#8a5a13' }, { from: '#230d18', to: '#6e1e3c' },
  { from: '#050505', to: '#2b2b31' }, { from: '#0d1b3d', to: '#1688fc' },
]
...
export function DimSlider({ value, onChange, label = 'Dim', hint }: {
  value: number
  ...
  return (
    <div className="fld dim-fld">
      ...
      <input
        id={id}
        className="dim-range"
        type="range" min={0} max={0.85} step={0.05} value={value}
...
export default function BgPicker({ value, onChange, image = false, noneLabel = 'none' }: {
  ...
  const modes = (image ? ['none', 'color', 'gradient', 'image'] : ['none', 'color', 'gradient']) as ('none' | 'color' | 'gradient' | 'image')[]
  return (
    <div className="bg-editor">
        ...
        {modes.map((t) => (
          <button
            ...
            onClick={() => {
              ...
              if (t === 'color') onChange({ type: 'color', color: bg.type === 'color' ? bg.color : '#101623' })
              if (t === 'gradient') onChange({ type: 'gradient', from: '#0b1026', to: '#12325e', angle: 135 })
              if (t === 'image') onChange({ type: 'image', url: '', dim: 0.45 })
            }}
          >
            {t === 'none' ? noneLabel : t}
          </button>
      ...
      {bg.type === 'color' && (
        <div className="fld row">
          <input type="color" value={bg.color} onChange={(e) => onChange({ type: 'color', color: e.target.value })} />
          <input value={bg.color} onChange={(e) => onChange({ type: 'color', color: e.target.value })} />
        </div>
      )}
      ...
      {bg.type === 'gradient' && (
        <>
          <div className="fld row">
            <input type="color" value={bg.from} onChange={(e) => onChange({ ...bg, from: e.target.value })} />
            <input type="color" value={bg.to} onChange={(e) => onChange({ ...bg, to: e.target.value })} />
            <input
              type="number" className="angle" value={bg.angle ?? 135} title="Angle"
          ...
          <div className="swatches">
            {GRADIENTS.map((g, i) => (
              <button
                ...
                onClick={() => onChange({ type: 'gradient', ...g, angle: bg.angle ?? 135 })}
              />
            ))}
          </div>
        </>
      ...
      {bg.type === 'image' && (
        <>
          <div className="fld">
            <input placeholder="https://… image URL" value={bg.url} onChange={(e) => onChange({ ...bg, url: e.target.value })} />
          </div>
          <DimSlider value={bg.dim ?? 0.45} onChange={(dim) => onChange({ ...bg, dim })} />
        </>
      )}
    </div>
  )
