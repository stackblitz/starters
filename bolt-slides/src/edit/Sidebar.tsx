...
function Row({ slide, index, active, onSelect, onMenu }: {
  ...
  const openComments = comments.filter((c) => c.slide_id === slide.id && !c.resolved).length
...
export default function Sidebar() {
  ...
  const onDragEnd = (e: DragEndEvent) => {
    ...
    reorder(arrayMove(slides.map((s) => s.id), from, to))
  }
  ...
  return (
    <aside className="ed-side">
      <div className="ed-side-head">
        <span>Slides</span>
        <button className="ghost-btn" title="Add slide" onClick={() => setAddAt(slides.length)}>+ Add</button>
      </div>
          ...
          <SortableContext items={slides.map((s) => s.id)} strategy={verticalListSortingStrategy}>
            {slides.map((s, i) => (
              <Row
                ...
                onMenu={(e) => { e.preventDefault(); setMenu({ x: e.clientX, y: e.clientY, id: s.id }) }}
              />
            ))}
          </SortableContext>
        </DndContext>
