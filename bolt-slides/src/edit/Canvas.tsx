...
export default function Canvas() {
  ...
  const note = (msg: string) => { setFlash(msg); setTimeout(() => setFlash(null), 2600) }
  ...
  useEffect(() => { setDataOpen(false); setMenu(null) }, [current])
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      const pad = 48
      const scale = Math.min((el.clientWidth - pad) / vw, (el.clientHeight - pad) / vh)
      setD({ vw, vh, scale })
    }
    update()
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === 'ArrowDown' || e.key === 'ArrowRight' || e.key === 'PageDown') { e.preventDefault(); setCurrent(current + 1) }
      if (e.key === 'ArrowUp' || e.key === 'ArrowLeft' || e.key === 'PageUp') { e.preventDefault(); setCurrent(current - 1) }
    }
  ...
  if (!slide) {
    return <div className="ed-canvas" ref={boxRef}><div className="ed-empty">No slides — add one from the rail.</div></div>
  }
  ...
  return (
    <div className="ed-canvas" ref={boxRef}>
      <div className="ed-frame" style={{ width: d.vw * d.scale, height: d.vh * d.scale }} onContextMenu={onCtx}>
        <AnimatePresence mode="wait" initial={false}>
          <motion.div
            key={slide.id}
            className="ed-frame-inner"
            style={{
              width: d.vw, height: d.vh, transform: `scale(${d.scale})`,
      ...
      <div className="ed-canvas-nav">
        <button className="ghost-btn" data-tip="Previous slide" disabled={current === 0} onClick={() => setCurrent(current - 1)}>←</button>
        <span className="ed-counter">{current + 1} / {slides.length}</span>
        <button className="ghost-btn" data-tip="Next slide" disabled={current >= slides.length - 1} onClick={() => setCurrent(current + 1)}>→</button>
        <button
          className={'icon-btn' + (notesOpen ? ' on' : '')}
          data-tip="Speaker notes"
          aria-label="Edit speaker notes for this slide"
          aria-pressed={notesOpen}
          onClick={() => { setDataOpen(false); setNotesOpen((v) => !v) }}
        >
        ...
        <span className={'nav-status' + (busy ? ' busy' : '')} role="status" aria-live="polite">
          {busy ?? flash}
        </span>
        <button className="ghost-btn" data-tip="Download the deck as a PDF" onClick={onPdf}>Export PDF</button>
        <a className="icon-btn" data-tip="Present" href="/present" target="_blank" rel="noreferrer" aria-label="Open the presentation in a new tab">
      ...
      {notesOpen && (
        <div className="notes-pop">
          <NotesEditor
            key={slide.id}
            value={slide.notes}
            onChange={(text) => { if (text !== slide.notes) useStore.getState().patchSlide(slide.id, { notes: text }) }}
            onDone={() => setNotesOpen(false)}
            placeholder="Speaker notes — what to SAY on this slide."
          />
          <p className="notes-pop-hint">Shown in the presenter console while you present. Edits there come back here live.</p>
        </div>
      )}
      {menu && (
        <ContextMenu
          x={menu.x} y={menu.y}
          items={[{ label: 'Edit data…', onClick: () => { setNotesOpen(false); setDataOpen(true) } }]}
          onClose={() => setMenu(null)}
        />
      )}
      {dataOpen && hasDataSheet(slide) && (
        <LayoutDataSheet key={slide.id} slide={slide} onClose={() => setDataOpen(false)} />
      )}
    </div>
  )
}
