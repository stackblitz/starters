...
const TOKEN = /(==[^=]+==|\*\*[^*]+\*\*|\+\+[^+]+\+\+|~~[^~]+~~|_[^_]+_|\{c:[^}]+\}[\s\S]+?\{\/c\}|\{s:[^}]+\}[\s\S]+?\{\/s\})/g

/* whole-field alignment: a `{a:l|c|r}` prefix on the stored string */
export const ALIGN_RE = /\{a:([lcr])\}/
export const ALIGNS: Record<string, 'left' | 'center' | 'right'> = { l: 'left', c: 'center', r: 'right' }
export function splitAlign(text: string): { align: 'l' | 'c' | 'r' | null; rest: string } {
  const m = text.match(/^\{a:([lcr])\}/)
  return m ? { align: m[1] as 'l' | 'c' | 'r', rest: text.slice(m[0].length) } : { align: null, rest: text }
}

export const COLOR_RE = /^(#[0-9a-fA-F]{3,8}|accent)$/
export const clampEm = (n: number) => Math.min(4, Math.max(0.4, n))

export const colorValue = (c: string) => (c === 'accent' ? 'var(--primary)' : c)

function braceParts(seg: string, tag: 'c' | 's'): { value: string; inner: string } | null {
  ...
  return line.split(TOKEN).map((seg, i) => {
    if (seg.startsWith('{c:')) {
      ...
      if (p && COLOR_RE.test(p.value)) {
        return <span key={i} data-color={p.value} style={{ color: colorValue(p.value) }}>{renderLine(p.inner)}</span>
      }
    }
    if (seg.startsWith('{s:')) {
      ...
      if (p && Number.isFinite(em)) {
        return <span key={i} data-fs={em} style={{ fontSize: `${em}em` }}>{renderLine(p.inner)}</span>
      }
    }
    if (seg.length >= 4) {
      if (seg.startsWith('==') && seg.endsWith('==')) return <span key={i} className="accent-text">{renderLine(seg.slice(2, -2))}</span>
      if (seg.startsWith('**') && seg.endsWith('**')) return <strong key={i}>{renderLine(seg.slice(2, -2))}</strong>
      if (seg.startsWith('++') && seg.endsWith('++')) return <span key={i} data-size="up" style={{ fontSize: '1.3em' }}>{renderLine(seg.slice(2, -2))}</span>
      if (seg.startsWith('~~') && seg.endsWith('~~')) return <span key={i} data-size="down" style={{ fontSize: '0.75em' }}>{renderLine(seg.slice(2, -2))}</span>
    }
    if (seg.length >= 3 && seg.startsWith('_') && seg.endsWith('_')) return <em key={i}>{renderLine(seg.slice(1, -1))}</em>
...
export function balanceLines(text: string): string {
  ...
  for (let pass = 0; pass < 3; pass++) {
    const next = out.replace(WRAPPED, (whole, tag: string, val: string, inner: string) =>
      inner.includes('\n')
        ? inner.split('\n').map((line) => (line ? `{${tag}:${val}}${line}{/${tag}}` : '')).join('\n')
        : whole)
    if (next === out) break
    out = next
  }
...
export function renderRich(text: string | null | undefined): ReactNode {
  ...
  return <span data-align={align} style={{ display: 'block', textAlign: ALIGNS[align] }}>{body}</span>
}
...
function lineToHtml(line: string): string {
  return line.split(TOKEN).map((seg) => {
    if (seg.startsWith('{c:')) {
      const p = braceParts(seg, 'c')
      if (p && COLOR_RE.test(p.value)) return `<span data-color="${p.value}" style="color:${colorValue(p.value)}">${lineToHtml(p.inner)}</span>`
    }
    if (seg.startsWith('{s:')) {
      const p = braceParts(seg, 's')
      const em = p ? clampEm(parseFloat(p.value)) : NaN
      if (p && Number.isFinite(em)) return `<span data-fs="${em}" style="font-size:${em}em">${lineToHtml(p.inner)}</span>`
    }
    if (seg.length >= 4) {
      if (seg.startsWith('==') && seg.endsWith('==')) return `<span class="accent-text">${lineToHtml(seg.slice(2, -2))}</span>`
      if (seg.startsWith('**') && seg.endsWith('**')) return `<strong>${lineToHtml(seg.slice(2, -2))}</strong>`
      if (seg.startsWith('++') && seg.endsWith('++')) return `<span data-size="up" style="font-size:1.3em">${lineToHtml(seg.slice(2, -2))}</span>`
      if (seg.startsWith('~~') && seg.endsWith('~~')) return `<span data-size="down" style="font-size:0.75em">${lineToHtml(seg.slice(2, -2))}</span>`
    }
    if (seg.length >= 3 && seg.startsWith('_') && seg.endsWith('_')) return `<em>${lineToHtml(seg.slice(1, -1))}</em>`
    return esc(seg)
  }).join('')
}
