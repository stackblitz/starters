...
export function parseNotes(text: string): Block[] {
  ...
  for (const raw of text.replace(/\r/g, '').split('\n')) {
    ...
    if (head) { flush(); blocks.push({ kind: 'h', lines: [head[2]] }); continue }
    if (quote) {
      flush()
...
function inlineCode(text: string): ReactNode {
  return text.split(/(`[^`]+`)/g).map((seg, i) =>
    seg.startsWith('`') && seg.endsWith('`') && seg.length > 2
      ? <code className="note-code" key={i}>{seg.slice(1, -1)}</code>
...
function inline(text: string): ReactNode {
  return text.split(HL_TOKEN).map((seg, i) => {
    ...
    if (m && HEX.test(m[1])) return <mark className="note-hl" key={i} style={{ background: m[1] }}>{inlineCode(m[2])}</mark>
...
export function NotesView({ text }: { text: string }) {
  ...
  return (
    <>
      {blocks.map((b, i) => {
        if (b.kind === 'h') return <h3 className="note-h" key={i}>{inline(b.lines[0])}</h3>
        if (b.kind === 'quote') return <blockquote className="note-quote" key={i}>{b.lines.map((l, j) => <p key={j}>{inline(l)}</p>)}</blockquote>
        if (b.kind === 'ul') return <ul className="note-list" key={i}>{b.items.map((it, j) => <li key={j}>{inline(it)}</li>)}</ul>
        if (b.kind === 'ol') return <ol className="note-list" key={i}>{b.items.map((it, j) => <li key={j}>{inline(it)}</li>)}</ol>
        return <p className="note-p" key={i}>{b.lines.map((l, j) => <Fragment key={j}>{j > 0 && <br />}{inline(l)}</Fragment>)}</p>
      })}
    </>
  )
}
...
const codeHtml = (s: string) =>
  s.split(/(`[^`]+`)/g)
    .map((seg) => (seg.startsWith('`') && seg.endsWith('`') && seg.length > 2
      ? `<code>${richToHtml(seg.slice(1, -1))}</code>`
      : richToHtml(seg)))
    .join('')
const codeToHtml = (s: string) =>
  s.split(HL_TOKEN).map((seg) => {
    const m = HL_ONE.exec(seg)
    return m && HEX.test(m[1])
      ? `<span data-hl="${m[1]}" class="note-hl" style="background:${m[1]}">${codeHtml(m[2])}</span>`
      : codeHtml(seg)
  }).join('')
...
export function notesToHtml(text: string): string {
  ...
  return blocks.map((b) => {
    ...
    if (b.kind === 'quote') return `<blockquote>${b.lines.map(codeToHtml).join('<br>')}</blockquote>`
    if (b.kind === 'ul') return `<ul>${b.items.map((i) => `<li>${codeToHtml(i)}</li>`).join('')}</ul>`
    if (b.kind === 'ol') return `<ol>${b.items.map((i) => `<li>${codeToHtml(i)}</li>`).join('')}</ol>`
...
function inlineText(node: Node): string {
  ...
  if (node.dataset.hl && HEX.test(node.dataset.hl)) return `{hl:${node.dataset.hl}}${inner}{/hl}`
  if (node.dataset.color) return `{c:${node.dataset.color}}${inner}{/c}`
  return inner
}
const prefixLines = (s: string, p: string) => s.split('\n').map((l) => p + l).join('\n')
...
export function htmlToNotes(root: HTMLElement): string {
  ...
  for (const node of Array.from(root.childNodes)) {
    ...
    const tag = node.tagName
    if (/^H[1-6]$/.test(tag)) { out.push('# ' + inlineText(node).trim()); continue }
    if (tag === 'BLOCKQUOTE') { out.push(prefixLines(inlineText(node).trim(), '> ')); continue }
    if (tag === 'UL' || tag === 'OL') {
      const items = Array.from(node.querySelectorAll(':scope > li'))
      out.push(items.map((li, i) => (tag === 'UL' ? '- ' : `${i + 1}. `) + inlineText(li).replace(/\n/g, ' ').trim()).join('\n'))
      continue
    }
...
/* ── the WYSIWYG ───────────────────────────────────────────────────── */
type Cmd = { id: string; label: string; hint: string; run: () => void; icon: ReactNode }
/* text colours, and highlighter washes (translucent, so the text stays legible
   on either surface) */
const TEXT_COLORS = ['#ffffff', '#ef4444', '#f5b73a', '#4fe5b0', '#4aa8ff', '#c084fc']
const HL_COLORS = ['#f5b73a55', '#4fe5b04d', '#4aa8ff4d', '#f472b64d', '#c084fc4d']
...
export function NotesEditor({
  value, onChange, onDone, placeholder, style, className = '',
}: {
  value: string
  ...
  useEffect(() => {
    ...
    const onDown = (e: PointerEvent) => {
      ...
      if (!t?.closest?.('.note-wyg-pop') && !t?.closest?.('.note-wyg-colorbtn')) setPalette(null)
    }
  ...
  const wrap = (open: string) => {
    ...
    if (!sel || sel.isCollapsed) { ref.current?.focus(); return }
  ...
  const paint = (hex: string) => wrap(`<span data-color="${hex}" style="color:${hex}">`)
  const mark = (hex: string) => wrap(`<span data-hl="${hex}" class="note-hl" style="background:${hex}">`)
  ...
  const strip = () => {
    ...
    const box = document.createElement('div')
    box.appendChild(sel.getRangeAt(0).cloneContents())
    box.querySelectorAll('[data-hl], [data-color], .accent-text').forEach((el) => {
  ...
  const cmds: Cmd[] = [
    { id: 'b', label: 'Bold', hint: '⌘B', run: () => exec('bold'), icon: svg('M6.2 3.6h4.6a3.1 3.1 0 0 1 0 6.2H6.2zM6.2 9.8h5.4a3.3 3.3 0 0 1 0 6.6H6.2z') },
    { id: 'i', label: 'Italic', hint: '⌘I', run: () => exec('italic'), icon: svg('M8.6 3.6h7M4.4 16.4h7M12.4 3.6l-4.4 12.8') },
    { id: 'h', label: 'Heading', hint: '', run: () => exec('formatBlock', '<h3>'), icon: svg('M5 4v12M13 4v12M5 10h8') },
    { id: 'ul', label: 'Bullets', hint: '', run: () => exec('insertUnorderedList'), icon: svg('M7.5 5h9M7.5 10h9M7.5 15h9M3.6 5h.01M3.6 10h.01M3.6 15h.01') },
    { id: 'ol', label: 'Numbered', hint: '', run: () => exec('insertOrderedList'), icon: svg('M8 5h8.5M8 10h8.5M8 15h8.5M3 4.4h1v3M3 9.2h1.6L3 11.6h1.6M3 13.6h1.5v1.2H3.4v1.2H4.9') },
    { id: 'q', label: 'Callout', hint: '', run: () => exec('formatBlock', '<blockquote>'), icon: svg('M4 5v10M8 6.5h8M8 10h8M8 13.5h5') },
    { id: 'c', label: 'Code', hint: '', run: code, icon: svg('M7 6.5L3.5 10 7 13.5M13 6.5L16.5 10 13 13.5') },
    { id: 'x', label: 'Clear formatting', hint: '', run: () => exec('removeFormat'), icon: svg('M5 4.6h10M10 4.6l-2.2 10.8M12.4 12.4l4 4M16.4 12.4l-4 4') },
  ]
  const history: Cmd[] = [
    { id: 'undo', label: 'Undo', hint: '⌘Z', run: () => exec('undo'), icon: svg('M7.5 6.5L4 10l3.5 3.5M4 10h7.5a4 4 0 1 1 0 8H9') },
    { id: 'redo', label: 'Redo', hint: '⇧⌘Z', run: () => exec('redo'), icon: svg('M12.5 6.5L16 10l-3.5 3.5M16 10H8.5a4 4 0 1 0 0 8H11') },
  ]
  ...
  return (
    <div className={'note-wyg-wrap ' + className}>
      ...
      <div
        ...
        onClickCapture={(e) => { if (panned.current) { e.preventDefault(); e.stopPropagation(); panned.current = false } }}
        onWheel={(e) => { if (bar.current && Math.abs(e.deltaY) > Math.abs(e.deltaX)) bar.current.scrollLeft += e.deltaY }}
      >
        {cmds.slice(0, 2).map((c) => (
          <button key={c.id} className="note-wyg-btn" data-tip={c.hint ? `${c.label} (${c.hint})` : c.label} aria-label={c.label} onClick={c.run}>
            {c.icon}
          </button>
        ))}
        ...
        <div className="note-wyg-pop-wrap">
          <button
            ref={colorBtn}
            className={'note-wyg-btn note-wyg-colorbtn' + (paletteOpen ? ' on' : '')}
          ...
          >
            ...
            {svg(
              ...
              <rect x="3.6" y="16.2" width="12.8" height="2.6" rx="1.3" fill="var(--primary)" stroke="none" />,
            )}
          </button>
          {palette && (
            <div className="note-wyg-pop" role="dialog" aria-label="Colour and highlight" style={{ top: palette.y, left: palette.x }}>
              ...
              <div className="note-wyg-pop-row">
                <button
                  ...
                  onClick={() => { accent(); setPalette(null) }}
                />
                {TEXT_COLORS.map((hex) => (
                  <button
                    key={hex}
                    ...
                    aria-label={`Text ${hex}`}
                    onClick={() => { paint(hex); setPalette(null) }}
                  />
                ))}
              </div>
              ...
              <div className="note-wyg-pop-row">
                {HL_COLORS.map((hex) => (
                  <button
                    ...
                    onClick={() => { mark(hex); setPalette(null) }}
                  />
                ))}
                <button className="note-wyg-swatch is-none" data-tip="Remove colour" aria-label="Remove colour and highlight" onClick={() => { strip(); setPalette(null) }} />
              </div>
            </div>
          )}
        </div>
        ...
        {history.map((c) => (
          <button key={c.id} className="note-wyg-btn" data-tip={`${c.label} (${c.hint})`} aria-label={c.label} onClick={c.run}>
            {c.icon}
          </button>
        ))}
        {onDone && <button className="note-wyg-done" onClick={() => { commit(); onDone() }}>Done</button>}
      </div>
      <div
        ref={ref}
