...
export default function TableEditor({ slide }: { slide: SlideData }) {
  ...
  useLayoutEffect(() => {
    const update = () => {
      ...
      setRects({
        ...
        cols: Array.from(tbl.querySelectorAll('thead th')).map(
          (th) =>
            t.x +
            (th as HTMLElement).offsetLeft +
            (th as HTMLElement).offsetWidth / 2
        ),
        rows: Array.from(tbl.querySelectorAll('tbody tr')).map(
          (tr) =>
            t.y +
            (tr as HTMLElement).offsetTop +
            (tr as HTMLElement).offsetHeight / 2
        ),
      });
    };
    update();
      ...
      const onMove = (ev: MouseEvent) => {
        ...
        const els =
          kind === 'row'
            ...
            : (Array.from(
                tbl.querySelectorAll(`tr > *:nth-child(${i + 1})`)
              ) as HTMLElement[]);
        const cls =
          kind === 'row'
            ? side === 'before'
  ...
  const menuItems = (m: { kind: 'row' | 'col'; index: number }): MenuItem[] =>
    m.kind === 'row'
      ...
      : [
          {
            label: hl === m.index ? 'Remove highlight' : 'Highlight column',
            onClick: () =>
              write(
                [...columns],
                rows.map((r) => [...r]),
                hl === m.index ? null : m.index
              ),
          },
  ...
  const grip = (
    ...
    <span
      className={
        'tbl-grip ' + (kind === 'col' ? 'tbl-grip-col' : 'tbl-grip-row')
      }
      style={style}
  ...
  return (
    ...
    >
      ...
      {rects &&
        hover.col != null &&
        rects.cols[hover.col] != null &&
        grip('col', hover.col, { left: rects.cols[hover.col], top: rects.top })}
      {rects &&
        hover.row != null &&
        rects.rows[hover.row] != null &&
        grip('row', hover.row, {
          left: rects.left,
          top: rects.rows[hover.row],
        })}
