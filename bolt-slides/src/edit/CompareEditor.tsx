  ...
  useLayoutEffect(() => {
    const update = () => {
      ...
      setRects({
        ...
        cols: head
          ? Array.from(head.children).map(
              (c) =>
                g.x +
                (c as HTMLElement).offsetLeft +
                (c as HTMLElement).offsetWidth / 2
            )
          : [],
        rows: Array.from(grid.querySelectorAll('.cmp-row:not(.cmp-head)')).map(
          (r) =>
            g.y +
            (r as HTMLElement).offsetTop +
            (r as HTMLElement).offsetHeight / 2
        ),
      });
    };
  ...
  return (
    ...
    >
      <div
        className="cmp mat"
        ref={gridRef}
        style={{ ['--vcols' as never]: String(vcols) }}
      ...
      >
        <div className="cmp-row cmp-head">
          {cols.map((_, i) => (
            <div
              key={i}
              className={'cmp-cell cmp-h' + (i - 1 === hl ? ' hl' : '')}
            >
        ...
        {rows.map((r, ri) => (
          <div key={ri} className="cmp-row">
            <div className="cmp-cell cmp-label">
              <T path={`rows.${ri}.label`} placeholder="Feature" />
            </div>
            {r.values.map((v, vi) => (
              <div
                key={vi}
