  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (!res.ok) {
      if (res.reason === 'throttled') {
        ...
        setError('Could not reach the deck. Check the connection and try again.')
      } else {
    ...
    setLockedFor(0)
    try { await load() } catch { setError('Unlocked, but the deck failed to load. Try again.') }
    setBusy(false)
  }
  ...
  return (
    <main className="gate" role="main">
      <form className="gate-card" onSubmit={submit}>
        ...
        <p className="gate-body" id="gate-help">This link is protected. Enter the password you were given.</p>
        <label className="gate-label" htmlFor="gate-pass">Password</label>
        <input
          id="gate-pass"
          ref={inputRef}
        ...
        <button className="solid-btn gate-submit" type="submit" disabled={busy || !password || lockedFor > 0}>
          {lockedFor > 0 ? `Locked (${fmtWait(lockedFor)})` : busy ? 'Checking…' : 'Open deck'}
        </button>
      </form>
    </main>
  )
