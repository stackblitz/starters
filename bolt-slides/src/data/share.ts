...
export interface ShareLink { mode: ShareMode; token: string; hasPassword: boolean }
...
export function withShare(path: string): string {
  ...
  return `${base}${sep}k=${encodeURIComponent(shareToken)}${hash ? '#' + hash : ''}`
}
...
export async function unlockShare(password: string): Promise<UnlockResult> {
  ...
  if (res.status === 429) {
    ...
    return { ok: false, reason: 'throttled', retryAfter: body.retryAfter ?? (Number.isFinite(header) ? header : 600) }
  }
...
export async function shareInfo(): Promise<{ mode: ShareMode; hasPassword: boolean } | null> {
