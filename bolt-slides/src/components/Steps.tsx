...
export default function Steps({ kicker, title, items }: {
  kicker?: string; title?: string; items: Step[]; nav?: string; notes?: string
}) {
    ...
    <div className="slide">
      <div className="container">
        <Reveal>
          ...
          {title && <h2 className="headline" style={{ textAlign: 'center', marginInline: 'auto', marginBottom: 'clamp(26px,4.5vh,48px)' }}>{title}</h2>}
        </Reveal>
        <div className="steps" style={{ '--n': items.length } as CSSProperties}>
          {items.map((s, i) => (
            <motion.div
              ...
              transition={{ duration: 0.5, delay: 0.18 + i * 0.14, ease: [0.16, 1, 0.3, 1] }}
            >
              <div className="step-no-row">
                <span className="step-no">{String(i + 1).padStart(2, '0')}</span>
                <span className={'step-line' + (i === items.length - 1 ? ' end' : '')}>
                  <motion.span
                    initial={animate ? { scaleX: 0 } : false}
                    animate={{ scaleX: 1 }}
                    transition={{ duration: 0.6, delay: 0.34 + i * 0.14, ease: [0.16, 1, 0.3, 1] }}
                  />
                </span>
              </div>
              <h3>{s.title}</h3>
