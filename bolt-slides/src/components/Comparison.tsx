...
export default function Comparison({ cols, rows, highlight = 0 }: { cols: string[]; rows: CompRow[]; highlight?: number }) {
  ...
  return (
    <div ref={ref} className="cmp mat" style={{ '--vcols': vcols } as CSSProperties}>
      ...
      {rows.map((r, ri) => (
        <motion.div
          ...
          transition={{ duration: 0.45, delay: 0.1 + ri * 0.07, ease: [0.16, 1, 0.3, 1] }}
        >
          <div className="cmp-cell cmp-label">{r.label}</div>
          {r.values.map((v, vi) => (
            <div key={vi} className={'cmp-cell' + (vi === highlight ? ' hl' : '')}>
              {typeof v === 'boolean' ? (
                <span className={'cmp-chip ' + (v ? 'ok' : 'no')}>{v ? <Check /> : <Cross />}</span>
              ) : (
