...
export default function Table({ columns, rows, highlightCol, highlightRow, caption }: {
  columns: TableColumn[]
  rows: TableCell[][]
  highlightCol?: number
  highlightRow?: number
  ...
  const align = (c: TableColumn) => (typeof c === 'string' ? undefined : c.align)
  ...
  return (
    <div ref={ref}>
      <div className="dtable mat">
        <table>
          <thead>
            <tr>
              {columns.map((c, i) => (
                <th key={i} className={(i === highlightCol ? 'hl-col' : '') + alignClass(i)}>
          ...
          <tbody>
            {rows.map((r, ri) => (
              <motion.tr
                ...
                transition={{ duration: 0.4, delay: 0.08 + ri * 0.06, ease: [0.16, 1, 0.3, 1] }}
              >
                {r.map((cell, ci) => (
                  <td key={ci} className={(ci === highlightCol ? 'hl-col' : '') + alignClass(ci)}>{cell}</td>
                ))}
              </motion.tr>
            ))}
          </tbody>
      ...
      {caption && <div className="foot dtable-caption" style={{ maxWidth: 880, marginInline: 'auto' }}>{caption}</div>}
    </div>
  )
}
