...
function probe(cssVar: string, prop: 'color' | 'backgroundColor' = 'color'): [number, number, number] {
  const el = document.createElement('div')
  el.style.cssText = `position:absolute;visibility:hidden;${prop === 'color' ? 'color' : 'background-color'}:var(${cssVar})`
...
function landPoints(): Promise<Float32Array> {
  ...
  landCache = new Promise((resolve) => {
    ...
    img.onload = () => {
      ...
      const N = 12000, GA = Math.PI * (3 - Math.sqrt(5))
      const pts: number[] = []
      for (let i = 0; i < N; i++) {
        const y = 1 - (2 * i) / (N - 1)
        ...
        if (data[(v * W + u) * 4] > 128) pts.push(Math.cos(lat) * Math.sin(l), Math.sin(lat), Math.cos(lat) * Math.cos(l))
      }
...
function GlobeCanvas({ markers, arcs = [] }: { markers: GlobeMarker[]; arcs?: GlobeArc[] }) {
  ...
  useEffect(() => {
    ...
    let raf = 0
    let size = 0
    const marks = markers.map((m) => ({ v: toVec(m.location), s: m.size ?? 0.06 }))
    // arcs: lifted great-circle samples (slerp + radial lift), precomputed once
    const K = 44
    const arcPts = arcs.map((a) => {
      const A = toVec(a.from), B = toVec(a.to)
      const d = clamp01((A[0] * B[0] + A[1] * B[1] + A[2] * B[2] + 1) / 2) * 2 - 1
    ...
    const render = () => {
      const s = size * dpr, cx = s / 2, cy = s / 2, R = s * 0.4
      ...
      ctx.fillStyle = glow
      ctx.beginPath(); ctx.arc(cx, cy, G, 0, 7); ctx.fill()
      ...
      const cosP = Math.cos(a), sinP = Math.sin(a)
      ...
      if (pts) {
        ...
        for (let i = 0; i < pts.length; i += 3) {
          ...
          ctx.beginPath(); ctx.arc(cx + xr * R, cy - yr * R, d, 0, 7); ctx.fill()
        }
      }
      ...
      for (let mi = 0; mi < marks.length; mi++) {
        ...
        if (el) {
          ...
          el.style.transform = `translate(${lx.toFixed(1)}px, ${ly.toFixed(1)}px) translate(-50%, calc(-100% - 12px))`
          el.style.opacity = String(e * e)
        }
        if (zr <= 0) continue
        const mx = cx + xr * R, my = cy - yr * R
        ...
        ctx.fillStyle = halo
        ctx.beginPath(); ctx.arc(mx, my, mr * 3, 0, 7); ctx.fill()
    ...
    let last = 0
    const loop = (t: number) => {
      if (last && !reduce && !drag.current.down) phi += ((t - last) / 1000) * 0.22 // rad/s, frame-rate independent
      last = t
      render()
    ...
    const down = (e: PointerEvent) => { drag.current.down = true; drag.current.startX = e.clientX; canvas.style.cursor = 'grabbing' }
    const move = (e: PointerEvent) => { if (drag.current.down) drag.current.r = drag.current.rBase + (e.clientX - drag.current.startX) / 140 }
    const up = () => { drag.current.down = false; drag.current.rBase = drag.current.r; canvas.style.cursor = 'grab' }
  ...
  return (
    <div className="globe-wrap" ref={wrapRef}>
      ...
      {markers.map((m, i) => m.label ? (
        <div key={i} className="globe-label" ref={(el) => { labelRefs.current[i] = el }}>
...
export default function Globe({ kicker, title, body, markers, arcs, stats, flip }: {
  kicker?: string; title: ReactNode; body?: ReactNode
  markers: GlobeMarker[]; arcs?: GlobeArc[]; stats?: GlobeStat[]; flip?: boolean
    ...
    <div className="slide full">
      <div className={'split' + (flip ? ' flip' : '')}>
        <div className="split-body">
          {kicker && <Reveal><div className="kicker">{kicker}</div></Reveal>}
          <Reveal delay={0.08}>
            <h2 className="headline" style={{ fontSize: 'clamp(32px,4.6vw,54px)' }}>{title}</h2>
          </Reveal>
          {body && <Reveal delay={0.16}><div className="lead">{body}</div></Reveal>}
        ...
        <div className="split-media globe-media">
          {isStatic
            ? <div className="globe-wrap"><div className="globe-static" aria-hidden /></div>
