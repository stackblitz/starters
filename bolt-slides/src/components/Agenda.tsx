...
export default function Agenda({ kicker, title, items }: {
  kicker?: string; title?: string; items: AgendaItem[]; nav?: string; notes?: string
}) {
    ...
    <div className="slide">
      <div className="container">
        <Reveal>
          ...
          {title && <h2 className="headline" style={{ textAlign: 'center', marginInline: 'auto', marginBottom: 'clamp(24px,4vh,40px)' }}>{title}</h2>}
        </Reveal>
        <div className="agenda">
          {items.map((it, i) => {
            ...
            return (
              ...
              >
                <span className="agenda-no">{String(i + 1).padStart(2, '0')}</span>
