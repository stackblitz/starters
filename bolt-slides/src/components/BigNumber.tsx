...
export default function BigNumber({ kicker, value, caption, foot }: {
  kicker?: string; value: ReactNode; caption?: ReactNode; foot?: string
  nav?: string; notes?: string
}) {
  return (
    <div className="slide center">
      <Reveal>
        {kicker && <div className="kicker" style={{ marginBottom: 18 }}>{kicker}</div>}
      </Reveal>
      <Reveal delay={0.08}>
      ...
      {foot && (
        <Reveal delay={0.3}>
          <div className="foot" style={{ marginTop: 'clamp(16px,3vh,26px)' }}>{foot}</div>
        </Reveal>
      )}
    </div>
  )
