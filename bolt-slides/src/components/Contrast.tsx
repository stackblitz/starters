...
export type ContrastPanel = { label?: string; title?: string; points: ReactNode[] }

const Cross = () => (<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.2} strokeLinecap="round" strokeLinejoin="round"><path d="M7 7l10 10M17 7L7 17" /></svg>)
...
export default function Contrast({ kicker, title, left, right }: {
  kicker?: string; title?: string; left: ContrastPanel; right: ContrastPanel
  ...
  const panel = (p: ContrastPanel, lit: boolean, i: number) => (
    <motion.div
      ...
      transition={{ duration: 0.55, delay: 0.18 + i * 0.14, ease: [0.16, 1, 0.3, 1] }}
    >
      {p.label && <span className={'chip' + (lit ? '' : ' dim')}>{p.label}</span>}
      {p.title && <h3>{p.title}</h3>}
      <div className="con-points">
        {p.points.map((pt, pi) => (
          <div key={pi} className="con-point">{lit ? <Check /> : <Cross />}<span>{pt}</span></div>
        ))}
      </div>
    </motion.div>
  )
    ...
    <div className="slide">
      <div className="container">
        <Reveal>
          ...
          {title && <h2 className="headline" style={{ textAlign: 'center', marginInline: 'auto', marginBottom: 'clamp(26px,4.5vh,46px)' }}>{title}</h2>}
        </Reveal>
