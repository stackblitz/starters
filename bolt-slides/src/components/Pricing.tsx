...
const Check = () => (
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.6} strokeLinecap="round" strokeLinejoin="round"><path d="M5 12.5l4.5 4.5L19 6.5" /></svg>
)

export default function Pricing({ kicker, title, tiers }: {
  kicker?: string; title?: string; tiers: Tier[]; nav?: string; notes?: string
}) {
  ...
  return (
    <div className="slide">
      <div className="container">
        ...
        <div className="pricing" style={{ '--tiers': tiers.length } as CSSProperties}>
          {tiers.map((t, i) => (
            <motion.div
              key={i}
              ...
              transition={{ duration: 0.55, delay: 0.16 + i * 0.09, ease: [0.16, 1, 0.3, 1] }}
            >
              {t.highlight && <span className="tier-badge">{t.badge ?? 'Most popular'}</span>}
              ...
              {t.features && t.features.length > 0 && (
                <div className="tier-list">
                  {t.features.map((f, fi) => (
                    <div key={fi} className="tier-feat"><Check />{f}</div>
                  ))}
                </div>
              )}
            </motion.div>
