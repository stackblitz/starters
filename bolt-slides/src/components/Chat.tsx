...
export default function Chat({ kicker, title, name = 'Assistant', messages }: {
  kicker?: string; title?: string; name?: string; messages: ChatMessage[]
    ...
    <div className="slide">
      <div className="container">
        <Reveal>
          ...
          {title && <h2 className="headline" style={{ textAlign: 'center', marginInline: 'auto', marginBottom: 'clamp(22px,3.5vh,36px)' }}>{title}</h2>}
        </Reveal>
        <Reveal delay={0.1}>
          <div className="chat mat">
            <div className="chat-head"><span className="ddot" />{name}</div>
