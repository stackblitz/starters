...
function tintNumbers(s: string, key: number): ReactNode {
  ...
  return (
    <Fragment key={key}>
      {parts.map((p, k) => (k % 2 === 1 ? <span key={k} className="cw-num">{p}</span> : p))}
    </Fragment>
  )
}
    ...
    <>
      {segs.map((seg, i) =>
        i % 2 === 1
          ...
          : <Fragment key={i}>{seg.split(KW).map((s, j) => (j % 2 === 1 ? <span key={j} className="cw-kw">{s}</span> : tintNumbers(s, j)))}</Fragment>,
      )}
