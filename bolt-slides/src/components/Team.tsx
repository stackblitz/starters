...
export type Person = {
  name: ReactNode; role?: ReactNode; img?: string
  /** avatar initials — required when `name` isn't a plain string */
  initials?: string
}

export default function Team({ kicker, title, people }: {
  kicker?: string; title?: string; people: Person[]; nav?: string; notes?: string
}) {
  ...
  return (
    <div className="slide">
      <div className="container">
        ...
        <div className="team">
          {people.map((p, i) => {
            const initials = p.initials ?? (typeof p.name === 'string' ? p.name.split(/\s+/).map((w) => w[0]).slice(0, 2).join('').toUpperCase() : '')
            return (
              <motion.div
                key={i}
              ...
              >
                <span className="person-ava">{p.img ? <img src={p.img} alt={typeof p.name === 'string' ? p.name : ''} /> : initials}</span>
