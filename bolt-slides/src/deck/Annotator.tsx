...
export default function Annotator({
    ...
    (ctx: CanvasRenderingContext2D, now: number) => {
      ...
      if (raw.length > 2) {
        ...
        for (let i = 0; i < n; i++) {
          ...
          const nx = -dy / len,
            ny = dx / len;
          const w = 4.2 * (0.42 + 0.58 * Math.pow(i / (n - 1), 0.7));
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => setInked(true), 110);
  ...
  const toolbar = active ? (
    ...
    >
            ...
            <div className="ann-pop-row">
              {COLORS.map((swatch, index) => (
                <button
                  ...
                  title={`${COLOR_LABELS[index]} (${index + 1})`}
      ...
      <button
        className="ann-btn"
        title={canRedo ? 'Redo (⇧⌘Z)' : undefined}
        aria-label="Redo"
        disabled={!canRedo}
        onClick={redo}
      >
      ...
      <button
        className="ann-btn"
        title={strokes.current.length ? 'Clear slide (⌫)' : undefined}
