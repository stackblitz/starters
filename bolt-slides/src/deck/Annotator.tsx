...
export type Tool = 'pen' | 'highlighter' | 'laser' | 'line' | 'arrow' | 'rect' | 'ellipse' | 'eraser'
/* x/y are relative to the stroke's anchor box (0..1); w = width factor */
type Pt = { x: number; y: number; w?: number }
export type Stroke = {
  tool: Tool; color: string; size: number; points: Pt[]
  anchor?: string
...
const TOOLS: { id: Tool; label: string; key: string; path: string }[] = [
  { id: 'pen', label: 'Pen', key: 'P', path: 'M4 20h4L18 10a2 2 0 0 0-3-3L5 17z' },
  { id: 'highlighter', label: 'Highlighter', key: 'H', path: 'M4 20h5l8-8-4-4-9 9zM13 7l4 4' },
  { id: 'laser', label: 'Laser pointer', key: 'L', path: 'M12 3v3M12 18v3M3 12h3M18 12h3M6.3 6.3l2.1 2.1M15.6 15.6l2.1 2.1M17.7 6.3l-2.1 2.1M8.4 15.6l-2.1 2.1M12 10.5a1.5 1.5 0 1 0 0 3 1.5 1.5 0 0 0 0-3z' },
  ...
  { id: 'ellipse', label: 'Ellipse', key: 'O', path: 'M12 6c4.5 0 8 2.7 8 6s-3.5 6-8 6-8-2.7-8-6 3.5-6 8-6z' },
  { id: 'eraser', label: 'Eraser', key: 'E', path: 'M8 18l-4-4a2 2 0 0 1 0-3l7-7a2 2 0 0 1 3 0l4 4a2 2 0 0 1 0 3l-7 7zM7 17h11' },
]
const COLORS = ['var(--primary)', '#ffffff', '#ef4444', '#f5b73a', '#4aa8ff', '#c084fc']
const SIZES = [2, 4, 7, 12]
const ERASER_R = 14
const LASER_LIFE = 650 // ms the trail lingers behind the dot
...
export function loadAnnotations(): Record<number, Stroke[]> {
  try {
    ...
    for (const k of Object.keys(raw)) if (Array.isArray(raw[k])) out[Number(k)] = raw[k]
    return out
  } catch { return {} }
}
function saveAnnotations(store: Record<number, Stroke[]>) {
  try {
    ...
    if (Object.keys(keep).length) localStorage.setItem(LS_KEY, JSON.stringify(keep))
    else localStorage.removeItem(LS_KEY)
  } catch { /* quota / private mode — annotations just stay in memory */ }
}

/* ── content anchoring ─────────────────────────────────────────────── */
const stage = () => document.querySelector('.slide-stage')
const viewportBox = (): Box => ({ left: 0, top: 0, width: window.innerWidth, height: window.innerHeight })
...
function anchorAt(cx: number, cy: number): string {
  for (const el of document.elementsFromPoint(cx, cy)) {
    ...
    while (block && block !== stage() && getComputedStyle(block).display === 'inline') block = block.parentElement
...
function measureBox(s: Stroke): Box {
  ...
  if (s.abox) {
    const v = viewportBox()
    return { left: s.abox[0] * v.width, top: s.abox[1] * v.height, width: s.abox[2] * v.width, height: s.abox[3] * v.height }
  }
  return viewportBox()
}
...
function clientPts(s: Stroke): Pt[] {
  const r = strokeBox(s)
  return s.points.map((p) => ({ x: r.left + p.x * r.width, y: r.top + p.y * r.height, w: p.w }))
}
...
function geometrySignature(list: Stroke[]): string {
  const v = viewportBox()
...
function hits(s: Stroke, x: number, y: number, r: number) {
  ...
  for (let i = 0; i < pts.length - 1; i++) if (distToSeg(x, y, pts[i].x, pts[i].y, pts[i + 1].x, pts[i + 1].y) < r + w / 2) return true
  return false
}
/* shift-constrain: 45° steps for lines, a perfect square/circle for shapes */
function constrain(tool: Tool, a: Pt, b: Pt): Pt {
...
function cursorFor(tool: Tool, size: number, color: string) {
  if (tool === 'laser') {
  ...
  if (tool === 'eraser') {
    ...
    return svgCursor(
      `<svg xmlns="http://www.w3.org/2000/svg" width="${d}" height="${d}"><circle cx="${d / 2}" cy="${d / 2}" r="${ERASER_R}" fill="rgba(255,255,255,0.14)" stroke="white" stroke-width="1.5"/></svg>`,
      d / 2, d / 2,
    )
  }
  if (tool === 'pen' || tool === 'highlighter') {
    ...
    return svgCursor(
      `<svg xmlns="http://www.w3.org/2000/svg" width="${d}" height="${d}"><circle cx="${d / 2}" cy="${d / 2}" r="${r}" fill="${color}" fill-opacity="0.5" stroke="white" stroke-width="1.2"/></svg>`,
      d / 2, d / 2,
    )
  }
  return 'crosshair'
...
export default function Annotator({
  slide, store, active, onDone, hold = false,
}: {
  slide: number
  ...
  const raf = useRef(0)
  const lastIn = useRef<{ x: number; y: number; t: number; w: number } | null>(null)
  ...
  const resolve = (c: string) =>
    c.startsWith('var(') ? (getComputedStyle(document.documentElement).getPropertyValue('--primary').trim() || '#4fe5b0') : c
  ...
  const paint = useCallback((ctx: CanvasRenderingContext2D, s: Stroke, p: Pt[]) => {
    ...
    if (s.tool === 'pen') {
      if (n === 1) { ctx.beginPath(); ctx.arc(p[0].x, p[0].y, Math.max(0.5, s.size / 2), 0, Math.PI * 2); ctx.fill(); ctx.restore(); return }
      ...
      const taper = (i: number) => 0.62 + 0.38 * Math.min(1, (i + 1) / 4, (n - i) / 4)
      for (let i = 0; i < n - 1; i++) {
        const a = p[i], b = p[i + 1]
        const from = i === 0 ? a : { x: (p[i - 1].x + a.x) / 2, y: (p[i - 1].y + a.y) / 2 }
        const to = { x: (a.x + b.x) / 2, y: (a.y + b.y) / 2 }
        ctx.lineWidth = Math.max(0.6, s.size * (((a.w ?? 1) + (b.w ?? 1)) / 2) * taper(i))
        ctx.beginPath(); ctx.moveTo(from.x, from.y); ctx.quadraticCurveTo(a.x, a.y, to.x, to.y); ctx.stroke()
      }
      const l = p[n - 1], pl = p[n - 2]
      ctx.lineWidth = Math.max(0.6, s.size * (l.w ?? 1) * 0.62)
      ctx.beginPath(); ctx.moveTo((pl.x + l.x) / 2, (pl.y + l.y) / 2); ctx.lineTo(l.x, l.y); ctx.stroke()
      ctx.restore(); return
    }
    ...
    if (s.tool === 'highlighter') {
      ...
      ctx.globalAlpha = 0.3; ctx.lineWidth = s.size * 3.2; ctx.lineCap = 'square'; ctx.lineJoin = 'round'
    ...
    if (s.tool === 'line' || s.tool === 'arrow') {
      ...
      if (s.tool === 'arrow') {
        const ang = Math.atan2(b.y - a.y, b.x - a.x), h = 9 + s.size * 1.9
        ctx.beginPath()
        ctx.moveTo(b.x - h * Math.cos(ang - 0.42), b.y - h * Math.sin(ang - 0.42))
        ctx.lineTo(b.x, b.y)
      ...
      ctx.beginPath(); ctx.roundRect ? ctx.roundRect(x, y, w, h, r) : ctx.rect(x, y, w, h); ctx.stroke()
  ...
  const paintLaser = useCallback((ctx: CanvasRenderingContext2D, now: number) => {
    ...
    if (raw.length > 2) {
      ...
      for (let i = 0; i < n; i++) {
        const p = sm[i], a = sm[Math.max(0, i - 1)], b = sm[Math.min(n - 1, i + 1)]
        const dx = b.x - a.x, dy = b.y - a.y
  ...
  const redraw = useCallback(() => {
    ...
    for (const s of strokes.current) if (s.tool === 'highlighter') paint(ctx, s, clientPts(s))
    for (const s of strokes.current) if (s.tool !== 'highlighter') paint(ctx, s, clientPts(s))
    boxCache = null
  ...
  /* ── history ─────────────────────────────────────────────────────── */
  const apply = useCallback((next: Stroke[], record = true) => {
    if (record) { past.current.push(strokes.current); future.current = [] }
  ...
  const clear = useCallback(() => { if (strokes.current.length) apply([]) }, [apply])
  ...
  const erase = useCallback((x: number, y: number) => {
    boxCache = new Map()
    const next = strokes.current.filter((s) => !hits(s, x, y, ERASER_R))
    boxCache = null
  ...
  const pumpLaser = useCallback(() => {
    ...
    const step = () => {
      ...
      if (laserHeld.current && laser.current.length) laser.current[laser.current.length - 1].t = now
  ...
  useEffect(() => {
    ...
    const watch = () => {
      ...
      const tick = () => {
        ...
        if (next !== sig) { sig = next; stableSince = now; redraw() }
        // keep watching until it has held still for 250ms (2.5s ceiling)
        watchRaf = (now - stableSince < 250 && now - start < 2500) ? requestAnimationFrame(tick) : 0
      }
      watchRaf = requestAnimationFrame(tick)
    }
    fit()
    watch()
    const ro = new ResizeObserver(() => { schedule(); watch() })
    const st = stage()
    if (st) ro.observe(st)
  ...
  const commitDraft = useCallback(() => {
    ...
    draft.current = null
    lastIn.current = null
    if (!d || !d.points.length) { schedule(); return }
    ...
    const v = viewportBox()
    d.abox = [r.left / v.width, r.top / v.height, r.width / v.width, r.height / v.height]
    d.points = d.points.map((p) => ({ x: (p.x - r.left) / r.width, y: (p.y - r.top) / r.height, w: p.w }))
  ...
  const endSession = useCallback(() => {
    ...
    if (base && base !== strokes.current) { past.current.push(base); future.current = []; forceHistory((v) => v + 1) }
  ...
  useEffect(() => {
    ...
    return () => { window.removeEventListener('pointerup', endSession); window.removeEventListener('pointercancel', endSession) }
  }, [endSession])

  // leaving draw mode mid-stroke still commits what was drawn
  useEffect(() => { if (!active) { endSession(); setStyleOpen(false); laser.current = [] } }, [active, endSession])

  // click anywhere else closes the style popover
  useEffect(() => {
    if (!styleOpen) return
    const onDown = (e: PointerEvent) => {
      if (!(e.target as HTMLElement | null)?.closest?.('.ann-style')) setStyleOpen(false)
    }
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      if (hit) { e.preventDefault(); setTool(hit.id); return }
      const n = parseInt(e.key, 10)
      if (n >= 1 && n <= COLORS.length) { e.preventDefault(); setColor(COLORS[n - 1]); return }
      if (e.key === '[' || e.key === ']') {
        e.preventDefault()
        setSize((s) => SIZES[Math.max(0, Math.min(SIZES.length - 1, SIZES.indexOf(s) + (e.key === ']' ? 1 : -1)))])
        return
      }
      if (e.key === 'Backspace' || e.key === 'Delete') { e.preventDefault(); clear() }
    }
  ...
  function widthFactor(e: { clientX: number; clientY: number; pressure: number; pointerType: string }) {
    const now = performance.now()
    ...
    if (e.pointerType === 'pen' && e.pressure > 0 && e.pressure !== 0.5) {
      ...
      const v = Math.hypot(e.clientX - prev.x, e.clientY - prev.y) / Math.max(4, now - prev.t) // px/ms
  ...
  function down(e: React.PointerEvent<HTMLCanvasElement>) {
    ...
    setDrawingNow(true)
    if (t === 'laser') { laserHeld.current = true; laser.current = [{ x: e.clientX, y: e.clientY, t: performance.now() }]; pumpLaser(); return }
    if (t === 'eraser') { eraseBase.current = strokes.current; erase(e.clientX, e.clientY); return }
    lastIn.current = null
    draft.current = { tool: t, color: colorRef.current, size: sizeRef.current, points: [{ x: e.clientX, y: e.clientY, w: widthFactor(e) }] }
    schedule()
  }
  ...
  function move(e: React.PointerEvent<HTMLCanvasElement>) {
    ...
    if (t === 'laser') {
      ...
      if (laser.current.length > 160) laser.current.splice(0, laser.current.length - 160)
      pumpLaser(); return
    }
    if (t === 'eraser') { if (e.buttons) erase(e.clientX, e.clientY); return }
    ...
    if (d.tool === 'pen' || d.tool === 'highlighter') {
      ...
      for (const ev of pts) {
        const w = widthFactor({ clientX: ev.clientX, clientY: ev.clientY, pressure: ev.pressure ?? 0.5, pointerType: e.pointerType })
        d.points.push({ x: ev.clientX, y: ev.clientY, w })
      }
    } else {
      // hold Shift for 45° lines and perfect squares / circles
      const end = e.shiftKey ? constrain(d.tool, d.points[0], { x: e.clientX, y: e.clientY }) : { x: e.clientX, y: e.clientY }
  ...
  const styleTool = tool === 'pen' || tool === 'highlighter' || tool === 'line' || tool === 'arrow' || tool === 'rect' || tool === 'ellipse'
  ...
  return (
    <>
      ...
      />
      {active && (
        <div className={'ann-bar' + (drawingNow ? ' drawing' : '')} onPointerDown={(e) => e.stopPropagation()}>
          ...
          <div className="ann-style">
            ...
            >
              <span className="ann-swatch-dot" style={{ background: color, width: Math.max(8, size + 4), height: Math.max(8, size + 4) }} />
            </button>
            {styleOpen && (
                ...
                <div className="ann-pop-row ann-pop-sizes">
                  {SIZES.map((s) => (
                    <button
                      ...
                      onClick={() => setSize(s)}
                    >
                      <span style={{ width: s + 3, height: s + 3, background: color }} />
                    </button>
                  ))}
                </div>
                <div className="ann-pop-hint">{styleTool ? '1–6 colors · [ ] size' : 'Pick a drawing tool to use these'}</div>
              </div>
            )}
          </div>
          ...
          <button className="ann-btn" data-tip="Undo (⌘Z)" aria-label="Undo" disabled={!canUndo} onClick={undo}><IconUndo /></button>
          <button className="ann-btn" data-tip="Redo (⇧⌘Z)" aria-label="Redo" disabled={!canRedo} onClick={redo}><IconRedo /></button>
          <button className="ann-btn" data-tip="Clear slide (⌫)" aria-label="Clear this slide" disabled={!strokes.current.length} onClick={clear}><IconTrash /></button>
          {onDone && (
            <>
              <span className="ann-sep" />
              <button className="ann-btn ann-done" data-tip="Done (D)" aria-label="Exit drawing mode" onClick={onDone}><IconDone /></button>
            </>
          )}
        </div>
      )}
