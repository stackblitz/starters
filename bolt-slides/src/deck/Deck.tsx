...
export default function Deck({ children, autoplay = false, stagger = 0.16 }: { children: ReactNode; autoplay?: boolean; stagger?: number }) {
  ...
  const [noteOverrides, setNoteOverrides] = useState<Record<number, string>>(() => {
    try { return JSON.parse(localStorage.getItem('deck:notes') || '{}') } catch { return {} }
  })
  ...
  const prev = useCallback(() => {
    ...
    if (slide > 0) { const n = slide - 1; const m = auto ? 0 : (maxMap.current[n] || 0); setSlide(n); setClicks(m); setCurMax(m) }
  }, [auto, clicks, slide])
  ...
  const setNote = useCallback((text: string) => {
    setNoteOverrides((prev) => {
      ...
      try { localStorage.setItem('deck:notes', JSON.stringify(nextO)) } catch { /* ignore */ }
      return nextO
    })
  }, [])
  const openPresenter = useCallback(() => {
    if (isPresenter) return
    const url = window.location.pathname + '?presenter=1' + window.location.hash
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        ...
        case 'h': case 'H': setUiHidden((v) => !v); break
        case 'Escape': setRailOpen(false); setDrawing(false); setUiHidden(false); break
      }
    }
  ...
  useEffect(() => {
    ...
    chan.current = c
    c.onmessage = (e) => {
      if (e.data?.type === 'state') { applyingRemote.current = true; setSlide(e.data.slide); setClicks(e.data.clicks); setAuto(e.data.auto) }
    }
    return () => c.close()
  }, [])
  useEffect(() => {
  ...
  return (
    <MotionConfig reducedMotion="user">
      <div className={'deck' + (cursorHidden ? ' nocursor' : '')}>
        ...
        {showAnnotator && <Annotator key={slide} slide={slide} store={annStore.current} active={drawing} />}

        <aside className={'noir-rail' + (railOpen ? ' open' : '')}>
          <div className="noir-rail-head">
            <span className="noir-rail-title">Slides</span>
            <button className="noir-icon-btn sm" data-tip="Close" onClick={() => setRailOpen(false)}><IconClose /></button>
          </div>
        ...
        {isPresenter && (
          <div className="noir-presenter">
            ...
            <textarea className="noir-presenter-notes" value={noteText} spellCheck={false} placeholder={'No notes — type here, or add notes="…" in code.'} onChange={(e) => setNote(e.target.value)} />
            <div className="noir-presenter-hint">Notes you type are saved on this device.</div>
          </div>
        )}

        <div className={'noir-dock' + (hideUI ? ' hidden' : '')}>
          <div className="noir-bar">
            <button className={'noir-icon-btn' + (railOpen ? ' on' : '')} data-tip="Overview (O)" onClick={() => setRailOpen((v) => !v)}><IconGrid /></button>
            <span className="noir-sep" />
            <button className="noir-icon-btn" data-tip="Previous" disabled={!hasPrev} onClick={prev}><IconLeft /></button>
            <div className="noir-counter"><span className="noir-counter-now">{slide + 1}</span><span className="noir-counter-tot">/ {total}</span></div>
            <button className="noir-icon-btn" data-tip="Next" disabled={!hasNext} onClick={next}><IconRight /></button>
            <span className="noir-sep" />
            <button className={'noir-icon-btn' + (auto ? ' on' : '')} data-tip="Auto-play (A)" onClick={toggleAuto}><IconAuto /></button>
            <button className={'noir-icon-btn noir-optional' + (drawing ? ' on' : '')} data-tip="Annotate (D)" onClick={() => setDrawing((v) => !v)}><IconPencil /></button>
            <button className="noir-icon-btn" data-tip={fs ? 'Exit fullscreen (F)' : 'Fullscreen (F)'} onClick={toggleFs}>{fs ? <IconShrink /> : <IconExpand />}</button>
            <button className="noir-icon-btn noir-optional" data-tip="Presenter — new tab (P)" onClick={openPresenter}><IconPresent /></button>
          </div>
        </div>
      </div>
    </MotionConfig>
