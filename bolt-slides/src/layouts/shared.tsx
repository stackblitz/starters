...
export interface FieldSpec {
  path: string
  label: string
  kind: 'text' | 'textarea' | 'select' | 'number' | 'toggle' | 'image' | 'list' | 'cardbg' | 'dim'
...
export const textField = (path: string, label: string): FieldSpec => ({ path, label, kind: 'text' })
export const headerFields = (): FieldSpec[] => [textField('kicker', 'Kicker'), textField('title', 'Title')]
...
export function Num({ path, value }: { path: string; value?: string }) {
  ...
  if (c) return <CountUp to={c.to} prefix={c.prefix} suffix={c.suffix} decimals={c.decimals} />
  return <T path={path} placeholder="42%" />
}

/** the standard centered kicker + headline block used by block-style layouts */
export function Heading({ slide, tight }: { slide: SlideData; tight?: boolean }) {
  const show = useShow()
  if (!show(slide.props.kicker) && !show(slide.props.title)) return null
  return (
    <Reveal>
      {show(slide.props.kicker) && (
        <div className="kicker" style={{ marginBottom: 10, textAlign: 'center' }}><T path="kicker" placeholder="Kicker" /></div>
      )}
      {show(slide.props.title) && (
        <h2 className="headline" style={{ textAlign: 'center', marginInline: 'auto', marginBottom: tight ? 'clamp(18px,3vh,30px)' : 'clamp(24px,4vh,44px)' }}>
...
/** split "a | b | c" cells */
export const pipe = (s: string | undefined) => (s ?? '').split('|').map((x) => x.trim())
...
export const normTable = (p: any): { columns: string[]; rows: string[][] } => ({
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  rows: (p.rows ?? []).map((r: any) => (Array.isArray(r) ? r : pipe(r?.cells ?? ''))),
})
