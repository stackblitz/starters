...
import { type LayoutDef, type FieldSpec, headerFields, textField, useShow, Heading, pipe, rich, normTable } from '@/layouts/shared'
...
const TimelineDef: LayoutDef = {
  ...
  defaults: {
    kicker: 'Roadmap', title: 'Where this goes.',
    items: [
      { time: 'Q3', title: 'Launch', body: 'Starter ships with the skill.' },
      { time: 'Q4', title: 'Collaboration', body: 'Live cursors and multiplayer editing.' },
      { time: '2027', title: 'Templates', body: 'A gallery of themed starting points.' },
    ],
  },
  fields: [...headerFields(), listField('items', 'Milestones', [
    textField('time', 'Time'), textField('title', 'Title'), { path: 'body', label: 'Body', kind: 'textarea' },
  ], TL_BLANK)],
  Render: ({ slide }) => (
    <Slide>
      <Heading slide={slide} />
      <Timeline
        items={(slide.props.items ?? []).map((it: Record<string, unknown>, i: number) => ({
          ...
          title: e(<LiCtl path="items" index={i} blank={TL_BLANK}><T path={`items.${i}.title`} /></LiCtl>),
...
const normCmp = (p: { cols?: unknown; rows?: unknown[] }) => ({
  cols: Array.isArray(p.cols) ? (p.cols as string[]) : pipe(p.cols as string | undefined),
  rows: ((p.rows ?? []) as { label?: string; values?: unknown }[]).map((r) => ({
    label: r.label ?? '',
    values: Array.isArray(r.values)
      ? (r.values as (boolean | string)[])
      : pipe((r.values as string | undefined) ?? '').map((v) => (v === 'yes' ? true : v === 'no' ? false : v)),
  })),
})
...
const ComparisonDef: LayoutDef = {
  ...
  defaults: {
    kicker: 'Why us', title: 'The honest comparison.',
    cols: ['', 'This starter', 'Slideware'], highlight: 0,
    rows: [
  ...
  fields: [...headerFields(),
    { path: 'highlight', label: 'Highlighted column (0-based)', kind: 'number' },
  ],
  Render: ({ slide }) => {
    const { editable, slideId } = useEdit()
    const setProp = useStore((s) => s.setProp)
    const legacy = !Array.isArray(slide.props.cols) ||
      (slide.props.rows ?? []).some((r: { values?: unknown }) => !Array.isArray(r?.values))
    useEffect(() => {
      ...
      <Slide>
        ...
        {canEdit ? (
          ...
          <Comparison
            ...
            rows={data.rows.map((r) => ({ label: rich(r.label) as unknown as string, values: r.values }))}
          />
        )}
      </Slide>
    )
...
const TableDef: LayoutDef = {
  ...
  defaults: {
    kicker: 'The data', title: 'By region.',
    ...
    highlightCol: 2, caption: 'Company data, FY26',
  },
  fields: [...headerFields(),
    ...
    { path: 'highlightCol', label: 'Highlight column (0-based, blank = none)', kind: 'number' },
  ...
  Render: ({ slide }) => {
    ...
    const setProp = useStore((s) => s.setProp)
    const show = useShow()
    const legacy = !Array.isArray(slide.props.columns) ||
    ...
    const labels = show(slide.props.labelLeft) && (
      <div className="tbl-labels">
        <span className="kicker"><T path="labelLeft" placeholder="Top-left label" /></span>
      </div>
    )
    const cls = [slide.props.filled && 'tbl-filled', slide.props.large && 'tbl-large'].filter(Boolean).join(' ')
    return (
        ...
        {canEdit ? (
          <>
            ...
            {show(slide.props.caption) && (
              <div className="foot dtable-caption" style={{ maxWidth: 880, marginInline: 'auto' }}>
...
const TabsDef: LayoutDef = {
  ...
  defaults: {
    kicker: 'One tool', title: 'Three audiences.',
    tabs: [
      { label: 'Founders', content: 'Pitch decks that stay on brand without a designer.' },
      { label: 'Product', content: 'Roadmaps and reviews, statuses on every slide.' },
      { label: 'Sales', content: 'Duplicate the master deck, tailor it per prospect.' },
    ],
  },
  fields: [...headerFields(), listField('tabs', 'Tabs', [
    textField('label', 'Label'), { path: 'content', label: 'Content', kind: 'textarea' },
  ], TAB_BLANK)],
  Render: ({ slide }) => {
    const { editable, slideId } = useEdit()
    const setProp = useStore((s) => s.setProp)
    ...
    <Slide>
      <Heading slide={slide} />
      <Tabs
        onAdd={editable && slideId ? () => setProp(slideId, 'tabs', [...(slide.props.tabs ?? []), structuredClone(TAB_BLANK)]) : undefined}
        tabs={(slide.props.tabs ?? []).map((t: { label: string; content: string }, i: number) => ({
          label: e(<T path={`tabs.${i}.label`} />),
          content: (
            <div className="lead" style={{ maxWidth: '52ch' }}>
              <LiCtl path="tabs" index={i} blank={TAB_BLANK}><T path={`tabs.${i}.content`} block /></LiCtl>
            </div>
          ),
        }))}
      />
...
const QA_BLANK = { q: 'A question people actually ask?', a: 'The short, honest answer.' }
...
const QaDef: LayoutDef = {
  ...
  defaults: {
    ...
    items: [
      { q: 'How long does this take to set up?', a: 'Minutes — one prompt produces the working draft.' },
      { q: 'Can I change what it made?', a: 'Everything: text in place, layouts, backgrounds, motion.' },
      { q: 'Where does the data live?', a: 'One portable file you can copy, back up, or hand off.' },
      { q: 'Is this only for pitches?', a: 'Any deck — reviews, all-hands, teaching, proposals.' },
    ],
  },
  fields: [textField('title', 'Title'),
    { path: 'large', label: 'Large rows', kind: 'toggle' },
    listField('items', 'Rows', [
    { path: 'q', label: 'Question', kind: 'textarea' },
    { path: 'a', label: 'Answer', kind: 'textarea' },
  ], QA_BLANK)],
  Render: ({ slide }) => (
    <Slide className={slide.props.large ? 'qa-wide' : undefined}>
      <h2 className="headline qa-title"><T path="title" placeholder="Title" /></h2>
      <div className={'qa-rows' + (slide.props.large ? ' large' : '')}>
        {(slide.props.items ?? []).map((_: unknown, i: number) => (
          <div key={i} className="qa-row">
            <div className="qa-q">
              <span className="qa-mark" aria-hidden>Q</span>
              <LiCtl path="items" index={i} blank={QA_BLANK}><T path={`items.${i}.q`} /></LiCtl>
            </div>
            <div className="qa-a">
              <span className="qa-mark qa-mark-a" aria-hidden>A</span>
              <T path={`items.${i}.a`} />
            </div>
          </div>
        ))}
...
const AccordionDef: LayoutDef = {
  ...
  defaults: {
    kicker: 'Questions', title: 'Asked and answered.',
    items: [
      { title: 'Where does the data live?', body: 'In one JSON file — data/deck.json. Copy it and everything travels.' },
      { title: 'Can I change what the AI made?', body: 'Everything: text in place, layouts, backgrounds, animations.' },
    ],
  },
  fields: [...headerFields(), listField('items', 'Items', [
    textField('title', 'Question'), { path: 'body', label: 'Answer', kind: 'textarea' },
  ], ACC_BLANK)],
  Render: ({ slide }) => (
    <Slide>
      <Heading slide={slide} />
      <Accordion
        items={(slide.props.items ?? []).map((_: unknown, i: number) => ({
          title: e(<LiCtl path="items" index={i} blank={ACC_BLANK}><T path={`items.${i}.title`} /></LiCtl>),
...
const ChatDef: LayoutDef = {
  ...
  defaults: {
    kicker: 'Prompt it', title: 'Slides, spoken into existence.',
    name: 'Bolt', messages: [
      { from: 'user', text: 'Make me a 10-slide seed pitch for a robotics startup.' },
      { from: 'ai', text: 'Done — cover, problem, product, traction, team, ask. Want the traction slide as a chart?' },
    ],
  },
  fields: [...headerFields(), textField('name', 'Window name'), listField('messages', 'Messages', [
    { path: 'from', label: 'From', kind: 'select', options: [{ value: 'user', label: 'User' }, { value: 'ai', label: 'AI' }] },
  ...
  Render: ({ slide }) => {
    ...
    return (
      <Chat
        ...
        messages={(slide.props.messages ?? []).map((m: { from: 'user' | 'ai' }, i: number) => ({
          from: m.from,
          text: e(<LiCtl path="messages" index={i} blank={MSG_BLANK}><T path={`messages.${i}.text`} block /></LiCtl>),
        }))}
      />
    )
  },
...
const CodeDef: LayoutDef = {
  ...
  defaults: {
    kicker: 'For developers', title: 'Author a deck as data.',
    filename: 'deck.json', code: '{\n  "layout": "bigNumber",\n  "props": { "value": "$3T", "caption": "the market" },\n  "animation": "cascade"\n}',
    highlight: '',
  },
  fields: [...headerFields(), { path: 'filename', label: 'Window title', kind: 'text', keep: true, plain: true },
    { path: 'code', label: 'Code', kind: 'textarea', keep: true, plain: true },
    { path: 'highlight', label: 'Highlight lines (e.g. 2,3)', kind: 'text', keep: true, plain: true },
  ],
  Render: ({ slide }) => {
        ...
        <div style={{ maxWidth: 760, marginInline: 'auto' }}>
          {editable ? (
            ...
            <CodeWindow
              ...
              highlight={String(slide.props.highlight ?? '').split(',').map((n) => parseInt(n.trim(), 10)).filter(Number.isFinite)}
            />
          )}
        </div>
      </Slide>
...
export const blockLayouts = [TimelineDef, ComparisonDef, TableDef, TabsDef, AccordionDef, QaDef, ChatDef, CodeDef]
