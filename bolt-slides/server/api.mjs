...
import { openDb, state, persist, getState, findSlide, sortedSlides, renumber, blankSlide, exportDeck, importDeck, uid } from './db.mjs'
import { access, may, listShares, saveShare, removeShare, unlock, shareInfo, sameOrigin, clientIp, throttledFor, MODES, MIN_PASSWORD } from './share.mjs'
...
const SLIDE_FIELDS = ['layout', 'props', 'background', 'animation', 'transition', 'nav', 'notes', 'status', 'assignee']

/* make room at `pos` by pushing everything from there down */
function shiftFrom(pos) {
  sortedSlides().forEach((s) => { if (s.position >= pos) s.position += 1 })
}
...
export function apiMiddleware(rootDir) {
  return async (req, res, next) => {
    ...
    try {
      ...
      if (m !== 'GET' && !sameOrigin(req)) return send(res, 403, { error: 'cross-origin' })
      const acc = access(req)
      const deny = () => {
        if (acc?.needsPassword) return send(res, 403, { error: 'password-required', mode: acc.mode })
      ...
      if (seg[0] === 'share') {
        if (m === 'GET' && seg.length === 1) {
          ...
          return info ? send(res, 200, info) : send(res, 404, { error: 'no such link' })
        }
        if (m === 'POST' && seg[1] === 'unlock') {
          ...
          if (wait) {
            ...
            return send(res, 429, { error: 'too-many-tries', retryAfter: wait })
          }
          const b = await readBody(req)
          const granted = unlock(b.token, b.password, ip)
          // a wrong password must not be distinguishable from a dead link
          return granted ? send(res, 200, granted) : send(res, 403, { error: 'denied' })
        }
      }
      if (seg[0] === 'shares') {
        ...
        if (m === 'PUT') {
          ...
          if (saved?.error === 'weak') return send(res, 400, { error: 'weak-password', min: MIN_PASSWORD })
      ...
      const visible = (state) => (acc.mode !== 'present' ? state : {
        ...state,
        slides: state.slides.map(({ notes, ...rest }) => ({ ...rest, notes: '' })),
      })

      /* ── state / deck ── */
      if (m === 'GET' && seg[0] === 'state') return send(res, 200, visible(getState()))
      if (m === 'PUT' && seg[0] === 'deck') {
        if (!writable('write')) return deny()
        const b = await readBody(req)
        const d = state().deck
        for (const k of ['title', 'transition', 'font', 'accent']) if (b[k] !== undefined) d[k] = b[k]
        d.updated_at = now()
        persist()
        return send(res, 200, { title: d.title, transition: d.transition, font: d.font, accent: d.accent })
      }
      ...
      if (m === 'PUT' && seg[0] === 'order') {
        if (!writable('write')) return deny()
        const b = await readBody(req)
        ;(b.ids ?? []).forEach((sid, i) => { const sl = findSlide(sid); if (sl) sl.position = i })
        persist()
        return send(res, 200, { ok: true })
      }
      ...
      if (m === 'POST' && seg[0] === 'profiles') {
        if (!writable('write')) return deny()
        const b = await readBody(req)
        const profile = { id: uid(), name: b.name, color: b.color, created_at: now() }
        state().profiles.push(profile)
        persist()
        return send(res, 201, { id: profile.id, name: profile.name, color: profile.color })
      }
      if (seg[0] === 'comments') {
        if (m !== 'GET' && !writable('write')) return deny()
        if (m === 'POST') {
          const b = await readBody(req)
          const comment = { id: uid(), slide_id: b.slideId, profile_id: b.profileId ?? null, body: b.body, resolved: 0, created_at: now() }
        ...
        if (m === 'PUT' && seg[1]) {
          ...
          if (c) { c.resolved = b.resolved ? 1 : 0; persist() }
          return send(res, 200, { ok: true })
        }
        if (m === 'DELETE' && seg[1]) {
          const s2 = state()
      ...
      /* ── portability ── */
      if (m === 'GET' && seg[0] === 'export') return send(res, 200, acc.mode === 'edit' ? exportDeck() : visible(exportDeck()))
      ...
      if (m === 'POST' && seg[0] === 'og') {
        ...
        fs.writeFileSync(path.join(rootDir, 'public', 'og.png'), Buffer.from(png, 'base64'))
        return send(res, 200, { ok: true })
      }
