...
const hash = (password, salt) =>
  crypto.scryptSync(password, salt, 32, SCRYPT).toString('hex');
...
const shareByToken = (t) =>
  state().shares.find((s) => s.token === String(t)) ?? null;
...
export function access(req) {
  ...
  if (share.pass_hash) {
    ...
    const grant = key
      ? state().grants.find((g) => g.key === String(key))
      : null;
    const fresh =
      grant &&
...
export function saveShare(mode, { password, rotate } = {}) {
  ...
  if (password === null || password === '') {
    share.pass_hash = null;
    share.pass_salt = null;
